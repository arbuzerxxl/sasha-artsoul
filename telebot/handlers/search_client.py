...
from telebot.keyboards.callbacks import client_callback
from telebot.settings import URL
from telebot.handlers.utils import authorization


@disp.callback_query_handler(client_callback.filter(action="search"))
async def process_search_client(query: types.CallbackQuery, state: FSMContext):
    """Поиск клиента в БД на основе API."""

    await query.message.delete_reply_markup()

    async with state.proxy() as state_data:
        if state_data['method'] == 'edit_client':
            await edit_user.EditUser.select_change.set()
        elif state_data['method'] == 'delete_client':
    ...
    if response.status_code == 200 and response.content:
        ...
        if not response_data:
            await state.finish()
            msg = f"<b>В данный момент список клиентов в БД пуст</b>"
            await query.message.answer(text=msg, parse_mode=types.ParseMode.HTML)
        else:
        ...
        await state.finish()
        msg = f"<code>Пользователей нет! [{response.status_code}]</code>"
        await query.message.answer(text=msg, parse_mode=types.ParseMode.HTML)
